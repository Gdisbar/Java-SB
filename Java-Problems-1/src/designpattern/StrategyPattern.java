...
class designpattern.PaymentStrategy {
        <<interface>>
    ...
    class designpattern.PaymentProcessor {
        -paymentStrategy: designpattern.PaymentStrategy
        +setPaymentStrategy(strategy: designpattern.PaymentStrategy) : void
        +processOrder(amount: float) : bool
    }

    designpattern.PaymentStrategy <|.. CreditCardPayment
    designpattern.PaymentStrategy <|.. PayPalPayment
    designpattern.PaymentStrategy <|.. CryptocurrencyPayment
    designpattern.PaymentProcessor -- designpattern.PaymentStrategy


 * **/

// abstract class designpattern.PaymentStrategy{ //
	...
	protected boolean processOrder(float amount)throws NumberFormatException{
		try{ // (amount==Math.round(amount))
			if(amount < 100.0f || amount > 1000.0f){
				...
				System.out.println("designpattern.CreditCard transaction amount "+amount);
				return true;
			}
	...
	protected boolean processOrder(float amount)throws NumberFormatException{
		try{ //(amount==Math.round(amount))
			if(amount > 0.0f  && amount > 500.0f){
				System.err.println("Amount needs to be less than Rs.500 for designpattern.PayPal transaction");
				return false;
			}else{
				System.out.println("designpattern.PayPal transaction amount "+amount);
				return true;
			}
		}catch(NumberFormatException e){
			System.err.println("Error in "+e.getMessage());
...
class PaymentProcessor{
	...
	// designpattern.PaymentStrategy strategy = new designpattern.PaymentStrategy();
