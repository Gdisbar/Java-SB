package Stream;/*
*
Time taken with sequential stream: 85 ms
Time taken with parallel stream: 35 ms
...
public class ParallelStreamBasic {
    public static void main(String[] args) {
        ...
        List<Integer> list = Stream.iterate(1, x -> x + 1).limit(200).toList();
        List<Long> factorialsList = list.stream().map(ParallelStreamBasic::factorial).toList();
        long endTime = System.currentTimeMillis();
        System.out.println("Time taken with sequential stream: " + (endTime - startTime) + " ms");

        startTime = System.currentTimeMillis();
        factorialsList = list.parallelStream().map(ParallelStreamBasic::factorial).toList();
